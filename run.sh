...
# Regular features
cat ./data/cub/meta.tsv | cut -d$'\t' -f2 | sed 's@^@./data/cub/images/@' | python -m tdesc --model vgg16 --crow --target-dim 448 > ./data/feats-crow-448

# Bilinear features
cat ./data/cub/meta.tsv | cut -d$'\t' -f2 | sed 's@^@./data/cub/images/@' | ./img2bilinear.py --outpath ./data/bilinear.bc

# run ./billy.py to compare performance
